...
const VOICE_PREVIEW_TEXT = 'Hi, thank you for calling. How may I help you today?';
...
// Cache for voice preview audio URLs to avoid re-generating
...
export function OnboardingWizard({ onComplete }: { onComplete: () => void }) {
  ...
  const playVoicePreview = async (voiceId: string) => {
    ...
    // If clicking same voice that's already playing, just stop
    ...
    try {
      let audioUrl = voicePreviewCache[voiceId];
      ...
      if (!audioUrl) {
        ...
        voicePreviewCache[voiceId] = audioUrl;
      }
      ...
      audio.onerror = () => {
        console.error('Audio playback error for voice:', voiceId);
        setPlayingVoice(null);
        setLoadingVoice(null);
        // Remove from cache if there was an error
        delete voicePreviewCache[voiceId];
      };
      ...
      setLoadingVoice(null);
      // Show a toast or message that voice preview is unavailable
    }
  };
  ...
  const searchPhoneNumbers = async () => {
    ...
    try {
      ...
      // Backend returns { numbers: [...], capabilities: {...}, message: string }
      ...
      if (numbers.length === 0) {
        ...
        // Show message if some capabilities weren't available
        if (response.message) {
          console.log('Phone search note:', response.message);
        }
      }
    } catch (err: any) {
  ...
  const handleSelectPhoneNumber = async (number: string) => {
    setSaving(true);
    try {
      // Save the selected number to the organization as a pending selection
      // This number will be purchased automatically once the user subscribes
      await api.patch('/api/organization', {
      ...
      console.error('Select phone error:', err);
    } finally {
      setSaving(false);
    }
  };
    ...
    switch (currentStep) {
      ...
      case 3: // Phone Number
        return (
          <div className="space-y-6">
            <p className="text-slate-400">
              Get a dedicated US phone number for your AI receptionist. You can skip this step and add a number later.
            </p>

            {/* Area Code Search - US Only */}
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">
              ...
              <div className="flex gap-3">
                ...
                <input
                  type="text"
                  value={data.areaCode}
                  onChange={(e) => setData({ ...data, areaCode: e.target.value.replace(/\D/g, '').slice(0, 3) })}
                  placeholder="e.g. 415"
                  maxLength={3}
                ...
                <Button 
                  ...
                  disabled={searchingNumbers || data.areaCode.length !== 3}
                >
                  {searchingNumbers ? <Loader2 size={18} className="animate-spin" /> : 'Search'}
                </Button>
              </div>
              <p className="mt-2 text-xs text-slate-500">
                Numbers include voice, SMS, and MMS capabilities when available.
              </p>
            </div>
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                ...
                {availableNumbers.map(num => (
                  ...
                  >
                    ...
                    <Button
                      onClick={() => handleSelectPhoneNumber(num.number)}
                      disabled={saving}
                    >
          ...
          <div className="text-center space-y-6">
            ...
            {data.phoneNumber ? (
              <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                <p className="text-sm text-blue-400 mb-1">Selected phone number</p>
                <p className="text-2xl font-bold text-white">{data.phoneNumber}</p>
                <p className="text-xs text-slate-400 mt-2">
                  This number will be purchased and activated when you subscribe to a paid plan.
                </p>
              </div>
            ) : (
              <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                <p className="text-sm text-amber-400">
                  No phone number selected yet. You can select one from the Phone Numbers page after subscribing.
                </p>
              </div>
            )}
